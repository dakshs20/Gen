        ...
        ['1:1', '4:5', '9:16', '16:9'].forEach(ratio => {
            ...
            if (radioInput) {
                radioInput.addEventListener('change', (e) => {
                    ...
                    updateImageWrapperAspectRatio(); // Update image display aspect ratio
                });
            }
...
function updateUI() {
    ...
    interactiveElements.forEach(el => {
        if (el) { // Only proceed if the element was successfully found
            ...
            const isGeneratorButton = el.id && (el.id === 'generate-image-btn'); // Only generate button is a "generator" button
            ...
            if (isAuthButton) {
                ...
                // Generate button disabled if auth not ready OR (not logged in AND no free generations) OR loading
                const noFreeGenerations = (!currentUser && freeGenerationsLeft <= 0);
                shouldBeDisabled = !isAuthReady || noFreeGenerations || loading;
            } else if (isEnhanceOrVariationButton) {
                // Enhance/Variation buttons disabled if auth not ready OR loading OR prompt is empty
                shouldBeDisabled = !isAuthReady || loadingEnhancePrompt || loadingVariationIdeas || !promptInput?.value.trim();
            } else if (el.id === 'use-enhanced-prompt-btn') {
                // Use Enhanced Prompt button is disabled if no enhanced prompt is available or loading
                shouldBeDisabled = !isAuthReady || !enhancedPrompt || loadingEnhancePrompt || loadingVariationIdeas;
            } else if (el.id === 'download-image-btn') {
                // Download button is disabled if no image is generated or loading
                shouldBeDisabled = !isAuthReady || !imageUrl || loading;
            } else if (el.id === 'copy-prompt-btn') {
                // Copy button is disabled if prompt input is empty
                shouldBeDisabled = !isAuthReady || !promptInput?.value.trim();
            } else if (el.id === 'clear-prompt-btn') {
                // Clear button is disabled if prompt input and negative prompt input are both empty
                shouldBeDisabled = !isAuthReady || (!promptInput?.value.trim() && !negativePromptInput?.value.trim());
            ...
            // For other general buttons (like navigation), they are enabled if isAuthReady is true
